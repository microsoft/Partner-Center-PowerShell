...
namespace Microsoft.Store.PartnerCenter.PowerShell
{
    ...
    public class PartnerService
    {
        ...
        /// </summary>
        private const string SdkConfiguration = "Microsoft.Store.PartnerCenter.PowerShell.Configuration.PartnerCenterSdk.json";
        ...
        internal static T SynchronousExecute<T>(Func<Task<T>> operation)
        {
            try
            {
                return Task.Run(async () => await operation().ConfigureAwait(false)).Result;
            }
        ...
        internal static void SynchronousExecute(Func<Task> operation)
        {
            try
            {
                Task.Run(async () => await operation().ConfigureAwait(false)).Wait();
            }
            catch (AggregateException ex)
            {
                throw ex.InnerException;
